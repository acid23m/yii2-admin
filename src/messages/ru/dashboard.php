<?php

return [
    // layout
    'panel' => 'Административная панель',
    'razrabotano na' => 'Разработано на',
    'razrabotchik' => 'Разработчик',
    'osnovnoe' => 'Основное',
    'glavnaya' => 'Главная',
    'privet' => 'Привет',

    // users
    'imya polzovatelya' => 'Имя пользователя',
    'parol' => 'Пароль',
    'rol' => 'Роль',
    'zapisi' => 'Записи',
    'posledniy vhod' => 'Последний вход',
    'prava' => 'Права',
    'avatar' => 'Аватар',
    'vkl 2fa' => 'Двух-факторная аутентификация по email',
    'zapomnit menya' => 'Запомнить меня',

    'imya polzovatelya moget sostoyat' => 'Имя пользователя может состоять из латинских заглавных и прописных букв, цифр и знака подчеркивания.',
    'parol moget sostoyat' => 'Пароль может состоять из латинских заглавных и прописных букв, цифр и символов ~!@#$%^&*()_',
    'neverniy polzovatel ili parol' => 'Неверное имя пользователя или пароль.',

    'polzovatel' => 'Пользователь',
    'polzovateli' => 'Пользователи',
    'profil' => 'Профиль',
    'neaktiven' => 'Неактивен',
    'aktiven' => 'Активен',

    'demo' => 'Демо',
    'avtor' => 'Автор',
    'moderator' => 'Модератор',
    'administrator' => 'Администратор',
    'superpolzovatel' => 'Суперпользователь',

    'polzovatel udalen' => 'Пользователь удалён.',
    'polzovatel dobavlen' => 'Пользователь добавлен.',
    'polzovatel obnovlen' => 'Пользователь обновлён.',
    'zametki sohraneny' => 'Заметки сохранены.',

    'dobavit polzovatelya' => 'Добавить пользователя',
    'obnovit polzovatelya' => 'Обновить пользователя:',
    'obnovit token' => 'Обновить API ID',

    // auth
    'dostup ogranichen' => 'Зона ограниченного доступа',
    'voyti' => 'Войти',
    'viyti' => 'Выйти',
    'vernutsa na sayt' => 'Вернуться на сайт',
    'vernutsa' => 'Вернуться',
    'na stranicu vhoda' => 'на страницу входа',
    'na glavnuyu sayta' => 'на главную сайта',
    'esli zabili parol' => 'Если забыли пароль, то вы можете его {sbrosit}.',
    'sbrosit' => 'сбросить',
    'dlya sozdaniya parola sleduyte po ssylke' => 'Для создания нового пароля следуйте по ссылке',
    'proverte pochtu i sleduyte instrukcii' => 'Проверте почту и следуйте инструкции.',
    'nevozmozhno otpravit soobshenie' => 'Невозможно отправить сообщение с инструкцией.',
    'polzovatel ne nayden' => 'Пользователь с таким почтовым адресом не найден.',
    'sbros parolya' => 'Сброс пароля',
    'noviy parol sohranen' => 'Новый пароль сохранён.',
    'noviy parol' => 'Новый пароль',
    'kluch ne mozhet bit pustim' => 'Ключ сброса пароля не может быть пустым.',
    'neverniy kluch sbrosa parolya' => 'Неверный ключ сброса пароля.',
    'zapros na sbros parolya' => 'Запрос на сброс пароля',
    'na ukazanniy adres budet vyslanna ssilka' => 'На указанный адрес будет выслана ссылка на сброс пароля.',
    'vi tochno hotite viyti?' => 'Вы действительно хотите выйти?',

    'odnorazoviy parol' => 'Одноразовый пароль',
    'neverniy kod' => 'Неверный код',
    'dvuh faktornaya' => 'Двух-факторная аутентификация',
    'proverte pochtu s kodom' => 'На почту отправлен одноразовый пароль.',

    // options
    'yazik adminki' => 'Язык админ.панели',
    'imya prilozheniya' => 'Имя приложения',
    'logo' => 'Логотип',
    'vremennaya zona' => 'Временная зона сервера',
    'dostup k saytu' => 'Доступ к сайту',
    'belie ip' => 'Белый список IP адресов',
    'chernie ip' => 'Черный список IP адресов',
    'teh pochta' => 'Техническая почта',
    'imya polzovatelya pochta' => 'Имя пользователя (Email)',
    'imya servera' => 'Имя сервера',
    'parol polzovatelya' => 'Пароль пользователя',
    'port' => 'Порт',
    'zashita soedineniya' => 'Защита соединения',

    'osnovnie nastroyki' => 'Основные настройки',
    'nastroyki obnovleni' => 'Настройки обновлены.',
    'pochtoviy shluz' => 'Почтовый smtp шлюз для отправки писем',

    'v doverennie ip' => 'Перед отключением необходимо внести доверенные IP в белый список',
    'spisok ip' => 'Список ip-адресов, разделенные запятой (и пробелом)',

    'kartinka kak data uri' => 'Формат изображения - data-uri (data:image/png;base64,)',

    'rezhim obsluzhivaniya' => 'Режим обслуживания',

    // metrika
    'metrika' => 'Сервисы Аналитики',
    'gugl analitika' => 'Идентификатор отслеживания Google Analytics',
    'ya metrika' => 'Номер счётчика Яндекс Метрики',

    // scripts
    'skripty' => 'Пользовательские скрипты',
    'razdel head' => 'Раздел <head> страницы',
    'razdel body' => 'Внизу страницы перед </body>',
    'zdes mozhno' => 'Здесь можно добавить дополнительный сторонний код, например счетчик, метрику, социальные кнопки, информационный виджет...',
    'buddte vnimatelni' => 'Будьте внимательны со скриптами, предоставленными третьими лицами. Код вписанный сюда, может повлиять на работоспособность приложения и создать угрозу безопасности!',

    // log
    'log oshibok' => 'Лог ошибок',
    'uroven' => 'Уровень',
    'kategoria' => 'Категория',
    'vremya' => 'Время',
    'prefiks' => 'Префикс',
    'soobshenie' => 'Сообщение',
    'log ochishen' => 'Лог очищен.',
    'zapisi loga udaleni' => 'Записи лога удалены',
    'skachat log' => 'Скачать лог',

    // server
    'tehnicheskie dannie' => 'Технические данные',
    'server' => 'Сервер',
    'host' => 'Хост',
    'os' => 'ОС',
    'processor' => 'Процессор',
    'yadra proc' => 'Ядра ЦПУ',
    'pamyat' => 'Память',
    'vsego' => 'Всего',
    'zanyato' => 'Занято',
    'svobodno' => 'Свободно',
    'disk' => 'Диск',

    // service
    'obsluzhivanie' => 'Обслуживание',
    'kesh' => 'Кэш',
    'sbrosit kesh vruchnuyu' => 'Не весь кэшируемый на сервере контент автоматически обновляется. Для обновления можно вручную сбросить кэш.',
    'sbrosit kesh' => 'Сбросить кэш',
    'kesh ochishen' => 'Кэш очищен.',
    'kesh ne skonfigurirovan' => 'Компонент Cache не сконфигурирован.',
    'papka resursov ochishena' => 'Папка ресурсов очищена.',
    'resursi' => 'Ресурсы',
    'direktoriya s resursami' => 'Директория с ресурсами - это временная папка со стилями, скриптами, изображениями и другими дополнительными файлами плагинов и расширений. Переодически её можно очищать.',
    'ochistit papku s resursami' => 'Очистить папку с ресурсами',
    'baza dannih' => 'База данных',
    'ostorozhno s bazoy' => 'Не рекомендуется прямое взаимодействие с базой данных. Используйте редактор, только если знаете, что делаете!',
    'redaktor bazy' => 'Редактор базы данных',
    'poiskoviy indeks' => 'Поисковый индекс',
    'nuzhno pereindeksirovat' => 'После добавления, обновления или удаления контента поисковую базу необходимо переиндексировать.',
    'obnovit indeks' => 'Обновить индекс',

    // trash
    'korzina' => 'Корзина',
    'gruppa' => 'Группа',
    'element' => 'Элемент',
    'ne mogu vosstanovit' => 'Не удалось восстановить.',

    // sitemap
    'karta sayta' => 'Карта сайта',
    'generirovat kartu' => 'Генерировать карту сайта',
    'sozdanie kart xml' => 'Файлы sitemap.xml помогают в индексировании сайта поисковиками. Главный файл sitemap_index.xml',

    // task
    'zadaniya po raspisaniyu' => 'Задания по расписанию',
    'zadaniya' => 'Задания',
    'minuta' => 'Минута',
    'chas' => 'Час',
    'den' => 'День',
    'mesac' => 'Месяц',
    'den nedeli' => 'День недели',
    'komanda' => 'Команда',

    // common
    'vremya sozdaniya' => 'Время создания',
    'vremya obnovleniya' => 'Время обновления',
    'status' => 'Статус',
    'pozicia' => 'Позиция',
    'izobrazhenie' => 'Изображение',
    'udalit izobrazhenie' => 'Удалить изображение',
    'opisanie' => 'Описание',
    'tekst' => 'Текст',
    'anons' => 'Анонс',
    'zagolovok' => 'Заголовок',
    'podzagolovok' => 'Подзаголовок',
    'nazvanie' => 'Название',
    'slag' => 'Url id страницы',
    'data' => 'Дата',
    'vremya' => 'Время',
    'dobavit' => 'Добавить',
    'obnovit' => 'Обновить',
    'otpravit' => 'Отправить',
    'sozdat' => 'Создать',
    'sohranit' => 'Сохранить',
    'udalit vse' => 'Удалить все',
    'udalit otmechennie' => 'Удалить отмеченные',
    'vosstanovit' => 'Восстановить из корзины',
    'dobavit zapis' => 'Добавить элемент',
    'obnovit zapis' => 'Обновить элемент',
    'sbrosit tablicu' => 'Сбросить таблицу',
    'vipolnaetsya' => 'Выполняется..',
    'perehod na novuyu stroku i abzac' => 'Переход на новую строку - Shift+Enter, переход на новый абзац - Enter. Рекомендуется вставлять уже готовый текст с помощью Ctrl+Shift+V (вставить только текст) и только форматировать его в данном редакторе.',
    'spisok tekstom' => 'Текстовый список - каждая запись с новой строки.',
    'zapis ne sushestvuet' => 'Запрошенный элемент не существует.',
    'zapis v korzine' => 'Элемент помещён в корзину.',
    'zapis vosstanovlena' => 'Элемент восстановлен из корзины.',
    'zapis udalena' => 'Элемент удалён.',
    'zapisi udaleni' => 'Элементы удалены.',
    'ne udaleno iz za svasannih dannih' => 'Невозможно удалить из-за связанных данных.',
    'zapis obnovlena' => 'Элемент обновлён.',
    'zapis dobavlena' => 'Элемент добавлен.',
    'gotovo' => 'Готово.',
    'opublikovana' => 'Опубликована',
    'opublikovan' => 'Опубликован',
    'udalena' => 'Удалена',
    'udalen' => 'Удалён',
    'chernovik' => 'Черновик',
    'zagolovok stranici' => 'Заголовок страницы',
    'opisanie stranici' => 'Описание страницы',
    'kluchevie slova' => 'Ключевые слова',
    'meta dannie' => 'Мета-данные страницы',
    'sortirovka' => 'Сортировка',
    'poisk' => 'Поиск',
    'resultat poiska' => 'Результаты поиска для: {query}'
];
